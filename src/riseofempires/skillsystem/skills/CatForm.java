...
public class CatForm extends AbstractSkill implements IToggleable{
	
	List<Player> activated = new ArrayList<>();

	public CatForm(SkillSystem system) {
		super(system, "CatForm", SkillType.TRANSMUTATION, SkillUsage.SELF, new ItemStack(Material.APPLE), 5, new StatPackage(0f).setCooldown(0).setManaCost(0).setDescription("Transform into a cat."));
	...
	public void toggle(Player player, SkillRarity rarity, int level) {
		if(isEnabled(player)) {
			...
			getMain().getCore().getUserManager().getUser(player).setMobDisguise(EntityType.OCELOT);
